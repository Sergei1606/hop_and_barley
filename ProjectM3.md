...
1. Введение и цель
Вы — разработчик на первой работе. Нужно трансформировать готовый статический HTML/CSS дизайн в полноценное интернет-магазин приложение на Django. Проект должен содержать:
...
2. Общие принципы
Веб-интерфейс (пользователи через браузер) использует session-based аутентификацию (стандарт Django).
...
Альтернативные менеджеры зависимостей (poetry, pip-tools/uv) приветствуются как бонус.
...
Линтеры и статическая проверка
...
Все зависимости задокументированы (requirements.txt или pyproject.toml).
...
Структура проекта
(Опционально) ссылка на деплой / screencast.
ФИО и группа + ссылка преподавателю.
Чек-лист реализации (отмеченное что сделано).
История коммитов / веток.
...
Неделя 1
Инициализация: Docker, PostgreSQL, шаблоны, статика.
Модели: Category, Product, Order, OrderItem, Review.
Каталог: список, фильтры, поиск, пагинация.
Неделя 2
Страница товара: отзывы, корзина.
Корзина: сессии, управление.
Оформление заказа: форма, email.
Личный кабинет: регистрация, история, редактирование.
Неделя 3
Админка: кастомизация, статистика.
REST API: ViewSets, JWT, документация.
Качество: линтеры, типизация, тесты, README.
(Бонус) GraphQL / CI / расширенные тесты.
Финальная проверка.